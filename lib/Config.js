...
Config.prototype.getAllByParameterName = function (parameterName) {

    var outputParameters = {};
    var keys = Object.keys(this._params);

    for (var i = 0; i < keys.length; i++) {
        if (this._params[keys[i]].param_name == parameterName) {
            outputParameters[this._params[keys[i]].kiosk_id] = this._params[keys[i]];
        }
    }
